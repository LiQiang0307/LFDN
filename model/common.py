...
class cycleBlock(nn.Module):
    def __init__(self, opt, nFeat=16):
        super(cycleBlock, self).__init__()
        negval = opt.negval
        conv = default_conv
        act = nn.ReLU(True)
        cycle_block = [nn.Sequential(
                nn.Conv2d(3, nFeat, kernel_size=3, stride=1, padding=1, bias=False),
                nn.LeakyReLU(negative_slope=negval, inplace=True)
            )]
        for i in range(4):
            cycle_block.append(RCAB(
                conv, nFeat , kernel_size=3, act=act))
        cycle_block.append(nn.Conv2d(nFeat, 3, kernel_size=3, stride=1, padding=1, bias=False))

        self.cycle_module = nn.Sequential(*cycle_block)

    def forward(self, x):
        x = self.cycle_module(x)
        return x
